...
class TestBasketView(TestCase):
    def setUp(self):
        ...
        self.client.post(
            reverse('basket_add'), {'productid': 1, 'productqty': 1, 'action': 'post'}, xhr=True
        )
        self.client.post(
            reverse('basket_add'), {'productid': 2, 'productqty': 2, 'action': 'post'}, xhr=True
        )
    ...
    def test_basket_add(self):
        response = self.client.post(
            reverse('basket_add'), {"productid": 3, "productqty": 1, "action": "post"}, xhr=True)
        self.assertEqual(response.json(), {'quantity': 4})

        response = self.client.post(
            reverse('basket_add'), {"productid": 2, "productqty": 1, "action": "post"}, xhr=True)
        self.assertEqual(response.json(), {'quantity': 3})

    def test_basket_delete(self):
        response = self.client.post(
            reverse('basket_delete'), {"productid": 2, "action": "delete"}, xhr=True)
        self.assertEqual(response.json(), {'quantity': 1, 'total': '400'})

    def test_basket_update(self):
        response = self.client.post(
            reverse('basket_update'), {"productid": 2, "productqty": 1, "action": "update"}, xhr=True)
